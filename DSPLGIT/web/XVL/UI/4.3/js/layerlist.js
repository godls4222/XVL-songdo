LayerList = function(params) {
	...
	this.update = function(params) {
		...
		var dom = Ext.ComponentQuery.query('layerlist[name=layerlist]')[0];
	...
	this.createLayerList = function() {
		...
		metaJsonLoaded[layerlistcacheurl].then(function() {
			...
			var dom = Ext.ComponentQuery
					.query('layerlist[name=layerlist]')[0];
			if (dom) {
				var columns0 = [];
				Ext.each(layerListParam.columns, function(column) {
					var val = {};
				...
				var sortNo = Number(layerListParam['sort']);
				var direction = layerListParam['order'];
				if (!isNaN(sortNo) && !Ext.isEmpty(direction)) {
					var idx = 0;
					Ext.each(layerListParam.columns, function(
							column) {
						if (idx == sortNo - 1) {
	...
	this.getLayerListData = function(val, jsonData) {
		...
		self.setChildData(jsonData.topLayer, val, layerPropSections,
				assemblyModelAttrs);
	}

	this.setChildData = function(childgroup, val, propSections) {
